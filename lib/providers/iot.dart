...
class IoTDevices with ChangeNotifier {
  ...
  List<Sensor> getListOfSensorsById(List<int> sensorIds) {
    return _cachedSensors.where((sensor) => sensorIds.contains(sensor.id)).toList();
  }
  ...
  Future<Sensor?> getAndReloadSensorById(int sensorId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        int index = _cachedSensors.indexWhere((sensor) => sensor.id == loadedSensor.id);
        _cachedSensors.replaceRange(index, index + 1, [loadedSensor]);
        notifyListeners();
        return loadedSensor;
      } else {
        return null;
